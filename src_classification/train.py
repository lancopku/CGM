...
def train(model, dataloader, scheduler, optim, updates):
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        # 还有score分数的信息
        logging("time: %6.3f, epoch: %3d, updates: %8d, train loss: %6.3f, train cca_loss:%6.3f, train acc: %.3f\n"
...
def eval(model, dataloader, epoch, updates, do_test=False):
    ...
    for span in tqdm(data, disable=not args.verbose):
        ...
        last_output = last_output.max(-1)[1]
        right_num = torch.sum((last_output == last_movement).float() * total_mask).item()
        # print('last movement', last_movement)
        if do_test:
            for test_output_item, test_label_item in zip(last_output.cpu().numpy().tolist(), last_movement.cpu().numpy().tolist()):
                logging_test([test_output_item, test_label_item])
        total_num += total_mask.sum().item()
        total_right += right_num
    acc = total_right / float(total_num)
    logging_csv([epoch, updates, acc])
    print('eval total movement number', total_num)
    print('evaluating accuracy %.2f' % (acc * 100))
    return acc
