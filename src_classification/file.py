...
class GraphSnap:
    def __init__(self, companies, date, node_num, is_hourly=False):
        ...
        self.node_headline = [['[START]'] for _ in range(node_num)]
    ...
    def add_node_text(self, node_id, news_list):
        if node_id not in self.node_headline:
            self.node_headline[node_id] = []
        for news in news_list:
            # id, headline, body, topic, rics, time
            n = News(news[0], news[1], news[2], news[3], news[4], news[5])
            # print(n.headline)
            self.node_headline[node_id].extend(word_tokenize(n.headline))
...
class GraphSpan:
    def __init__(self, span, is_hourly=False):
        ...
        if is_hourly:
            ...
            self.span_text = [d.node_headline for d in self.data[:-1]]
            ...
            self.hourly_movement = [node_movement for node_movement in self.data[:-1].movement]
            self.hourly_movement_mask = [data.get_movement_mask() for data in self.data]
        else:
            self.size = len(self.data)
            self.span_text = [d[1].node_headline for d in self.data]
    ...
    def convert_feature(span_features, is_hourly=False):
        ...
        for i in range(len(span_features)):
            ...
            for node_id in range(len(span_features[i])):
                if is_hourly:
                    assert len(span_features[i][node_id]) == 7, len(span_features[i][node_id])  # 6 source features
                    ...
                    day_features = []
                    assert len(span_features[i][node_id]) == 6, len(span_features[i][node_id])  # 6 source features
...
class Graph:
    ...
    def extract_data_span(self, is_hourly=False):
        dates = {}  # dict of GraphSnaps
        # TODO: should change a lot if not using daily prediction
            ...
            for company in self.companies:
                for date in self.companies[company].prices:
                    ...
                    for hour in range(5):
                        ...
                        last_movement = 0 if hour == 0 else self.companies[company].prices[date][0][hour - 1]
                        # 1 ~ 6 are the indices of features excluding movement
                        feature = [last_movement] + [self.companies[company].prices[date][i][hour] for i in range(1, 6)]
