...
class GraphSnap:
    ...
    def add_node_feature(self, node_id, feature, feature_scale):
        ...
        feature[6] = feature[5] / feature[6]  # volume_percentage
        feature[5] = scale(feature[5], feature_scale[2], feature_scale[3])  # volume
        feature.append(self.companies[node_id].judge_label(feature[6]))  # label of movement
        ...
        for company in self.companies:
            for prices_idx, date in enumerate(self.companies[company].prices):
                ...
                if judge_dataset(date, self.config) >= 0:  
                    ...
                    # from ipdb import set_trace; set_trace() 
                    snap.add_node_movement(self.com2id[company], today_open_volume/today_total_volume)
                    if dates[date][21] is None:
                        dates[date][21] = snap
                    # from ipdb import set_trace; set_trace() 
